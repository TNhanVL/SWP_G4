    ...
    <div class="box">
        <form action="/signup" method="post" id="signUpForm" class="needs-validation">
            ...
            <div class="inputBox">
                ...
                <input class="form-control <c:if test="${user.email != null}">is-valid</c:if>" id="email" type="text"
                       placeholder="Enter your email" required="required" name="email" value="${user.email}"
                       <c:if test="${user.email != null}">readonly</c:if>>
                <i></i>
            </div>
            <div class="inputBox-name">
                ...
                <div class="inputBox">
                    ...
                    <input class="form-control" type="text" placeholder="Last name" required="required" name="lastName"
                           value="${user.lastName}">
                    <i></i>
                </div>
            </div>
                ...
                <select class="form-control" name="countryID" id="country">
                    <c:forEach items="${countries}" var="country">
                        <option value="${country.ID}"
                                <c:if test="${country.ID == 16}">selected</c:if>>${country.name}</option>
                    </c:forEach>
                </select>
                <i></i>
            </div>
